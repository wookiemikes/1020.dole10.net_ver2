...
$columns = array('ud_id', 'ud_est_name', 'ud_email', 'ud_contact', 'ua_status', 'uad_date_filed','uad_ein', 'uad_date_approved', 'id_diff');
...
while ($row = mysqli_fetch_array($result)) {
    ...
    $sub_array[] = $row["ud_contact"];
    $sub_array[] = "<b style='color:".$color. "'><i style='color:#228B22;' class='mdi mdi-check-circle m-r-5'></i> ".$tag."</b>";
    $sub_array[] = $row["uad_date_approved"];
    $sub_array[] =
                    "<form action='../pages/dashboardview-approved' method='POST'>
                      <input type='hidden' name='id' value='" . $row["id_diff"] . "'>
					  <input type='submit' class ='btn btn-sm btn-info btn-block' name ='viewapproved' id = 'submit' value ='VIEW (?)'>
					</form> ";
    $data[] = $sub_array;
}
