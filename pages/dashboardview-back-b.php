...
if (isset($_POST['sendPendingBtnPfo'])) {
    ...
    if ($query1) {
        ...
        echo '<script>alert("Email failed successfully !")</script>';
        echo '<script>window.location.href="../pages/dashboard-pfo-view.php";</script>';
    }
}
...
if (isset($_POST['sendDenyBtnPfo'])) {
    ...
    $query = $connect->query("UPDATE `user_details_tbl` SET
        ua_status = 'DENIED' WHERE id_diff ='" .$id_diff. "';") or die($connect->error);
    $query1 = $connect->query("UPDATE `user_app_details` SET
        uad_remarks = '" .$remarks1. "' WHERE id_diff ='" .$id_diff. "'") or die($connect->error);;
        ...
        $remarks1 = '<!doctype html>
			...
			<span class="preheader" style="color: transparent; display: none; height: 0; max-height: 0; max-width: 0; opacity: 0; overflow: hidden; mso-hide: all; visibility: hidden; width: 0;">' . $remarks1 . '</span>
				<div class="container">
                        ...
                        <br/>
                        <br/>
                        Your application for the Rule 1020 Online Registration of Establishment has been DENIED. Reasons for this are :
                            ' .$remarks1. '<br/>
